...
class ReplayBuffer:
    ...
    def add(self, tensor_data):
        ...
        with self.file_lock:
            ...
            # Avoid re-writing to memory after each add, perform this explicitly when required
            # self.write_to_memory()

    def write_to_memory(self):
        """Write buffer data into memory (CPU)."""
        print_rank_0("Writing buffer data into memory.")
        self.data = []
        ...
        while True:
            if len(self.prefetch_queue) < self.prefetch_size:
                with self.prefetch_lock:
                    ...
                    indices = torch.randperm(len(buffer_data))[:self.prefetch_size]
                    selected_data = buffer_data[indices]

                    # Ensure selected_data is 2D before adding to queue
                    if selected_data.dim() == 1:
                        selected_data = selected_data.unsqueeze(0)
    ...
    def get_batch(self, buffer_proportion=0.5):
        """Fetch a batch from the prefetch queue."""
        ...
        with self.prefetch_lock:
            ...
            if available_samples > 0:
                # Ensure indices are valid integers and format is correct
                indices = torch.randperm(len(buffer_data))[:available_samples]

                # Ensure indices are used correctly for slicing
                selected_batch = [buffer_data[i] for i in indices]

                # Convert the list of tensors into a single tensor if needed
                selected_batch = torch.stack(selected_batch) if isinstance(selected_batch[0], torch.Tensor) else selected_batch

                # Ensure selected_batch is 2D before returning
                if selected_batch.dim() == 1:
                    selected_batch = selected_batch.unsqueeze(0)  # Shape: (1, seq_length)

                return selected_batch.to(self.device)
            else:
    ...
    def save_metadata(self, path):
        ...
        metadata = {
            ...
            'tensor_shape': self.tensor_shape if self.tensor_shape is not None else None,
        ...
        # Write buffer data to memory explicitly after saving metadata
        self.write_to_memory()
