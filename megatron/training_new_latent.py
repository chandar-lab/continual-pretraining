...
def pretrain(neox_args):
    ...
    # combined_model = BartEncoderPerceiverGPT2Pipe(
    #     bart_encoder=bart_model.encoder,
    #     perceiver_ae=perceiver_ae,
    #     gpt2_pipe=gpt2_pipe,
    #     bart_hidden_size=768,
    #     gpt2_hidden_size=1024,
    ...
    model, optimizer, lr_scheduler, reference_model = setup_model_and_optimizer(
        model=model, neox_args=neox_args, use_cache=False, iteration=neox_args.iteration
    )
    ...
    try:
        best_performances, best_performances_checkpoint=load_best_performances(neox_args.forget_path)
    except:
    ...
    for i in range(task_id, len(neox_args.train_data_paths)):
        _,valid_data_iterator, test_data_iterator= build_train_valid_test_data_iterators_blunded(
...
def get_neox_loss(maybe_tuple, labels, is_train, neox_args):
    """
    gpt neox decoder loss
    """
...
def forward_step(
    ...
    # ################################## TODO: add latent part here #######################################################

    # from contextlib import nullcontext
    # if neox_args.lm_mode == 'freeze':
    #     encoder_context = torch.no_grad()
    # else:
    #     encoder_context = nullcontext()
    
    # with encoder_context:
    #     encoder_outputs = model.get_encoder()(input_ids = tokens, attention_mask = attention_mask)
    #     encoder_outputs = model.encoder_output_to_decoder_input(encoder_outputs, attention_mask)
    # # latent_loss = bart_model(labels=data['labels'], encoder_outputs=encoder_outputs).loss # only if bart is also the decoder
    # maybe_tuple = model((encoder_outputs, position_ids, attention_mask), neox_args=neox_args)
    # latent_loss = get_neox_loss(maybe_tuple, labels, is_train, neox_args)
    ...
    loss = get_neox_loss(maybe_tuple, labels, is_train, neox_args)
    # add latent loss #
    loss += latent_loss
    ###
    if neox_args.memory_profiling:
...
def get_encoder_gpt2_model(neox_args, use_cache=False):
    """Build the model."""

    # Build model on cpu.
    print_rank_0("building encoder-GPT2 model ...")
...
def setup_model_and_optimizer(model, neox_args, use_cache=False, iteration=None):
